...
import { RootContent, LeftContent, RightContent, Root, Heading, Description, Text, DivCategory } from './styled';
        ...
        return (
            <Fragment key={index}>
                <div className="col-xs-6 col-md-3">
                    <DivCategory>
                        <a href="#">{item.name}</a>
                        <div className="iconCate">{item.icon ? <img src={item.icon} alt={item.name} /> : null}</div>
                        <div className="textBottom">{item.name}</div>
                    </DivCategory>
                </div>
            </Fragment>
        );
