...
class WidgetGiftCodeElement extends PureComponent {
    constructor() {
        super();
        this.state = {
            dataCourse: []
    ...
    componentDidMount() {
        ...
        if (promotionId) {
            widgetActions.listGiftCodeCourse(promotionId).then(data => {
                if (data) {
                    this.setState({ dataCourse: data && data.data && data.data.rows ? data.data.rows : [] });
                }
            });
        }
    }
...
WidgetGiftCodeElement.propTypes = {
...
export default connect(mapStateToProps, mapDispatchToProps)(WidgetGiftCodeElement);
