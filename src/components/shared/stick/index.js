...
export default class Stick extends PureComponent {
    ...
    onClose = event => {
        ...
        const thisRect = this.holder.current.getBoundingClientRect();
        ...
        const outOfThis =
            event.pageX < thisRect.left ||
            event.pageX > thisRect.left + thisRect.width ||
    ...
    render() {
        ...
        return (
            <WrapRoot className={cx(className)} style={style} innerRef={this.holder}>
                {children}
            </WrapRoot>
        );
    }
